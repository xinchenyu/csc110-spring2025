...
def main():
  ...
  assert find_min_index(test_list) == 3
...
Call your `find_min_index` function, find the index of smallest number starting from `begin_index`.
...
assert items[begin_index:] == [20, 7, 3, 9] # 3 is the min

# index of 3 in the sublist is 2
assert find_min_index(items[begin_index:]) == 2  
...
Now you found the index of smallest number starting from `begin_index`, let's make one swap: swap the items at `begin_index` and `min_index`.
